...
| S23 | Превышение глубины SVG | Отдельный безопасный CAPTURE_SVG_COMPLEXITY_LIMIT, без arbitrary exception text |
...
| B15 | Две панели, одинаковые SVG ID, разные цвета | Раздельные snapshots, state и actual paint; test-only registry не смешивает панели |
| B16 | Две ячейки одной SVG-строки, настоящее `label: replace` | UI и API сохраняют 95/red; оба индикатора связаны с Service Alpha, числа не становятся именами |
...
| V30 | Автоматический перенос названия | Один логический объект, resize не меняет смысл |
| V31 | Отдельные tspan targets и разноцветные части общего text | Сохраняются владельцы и цвета каждого фрагмента |
| V32 | Прозрачный текст и ненарисованная рамка | Не используются как основания визуальной связи |
| V33 | use со стилем, отличным от shadow content | Реальный paint не угадывается, appearance пуст |
| V34 | Malformed condition/color/refid/tooltip/title | Снимок валиден, добавлена CAPTURE_INVALID_VALUE, расчёт не меняется |
| V35 | Текст под clip-path | Недостоверное visible name не выдаётся; collector отмечает неопределённость |
| V36 | SVG href переопределён YAML | Исходный declared без ruleId и applied URL сохраняются отдельно |
| V37 | Подпись строки с невидимой authored рамкой и несколькими видимыми ячейками | Общая inferred группа только при повторяющихся ячейках строки; невидимая рамка сама по себе не связывает индикаторы |
| V38 | Две строки с динамическими числовыми ячейками | Каждый индикатор связан с подписью своей строки через inferred/row_alignment; числа не становятся именами, DOM и цвета не меняются |
| V39 | Значение стало пустым или изменилось; SVG уменьшен | Видимые рамки сохраняют принадлежность ячеек строкам независимо от текста значения и масштаба |
| V40 | Вторая ячейка скрыта, не окрашена, обрезана, нулевого размера либо рамки перекрываются | Недостаточно доказательств повторяющихся видимых ячеек; связь со строкой не выдумывается |
| V41 | Две подписи с одинаково подходящими областями строки | ambiguous с обеими кандидатурами, без произвольного выбора |
| V42 | Подпись HTML продолжается во вложенном блоке на следующей строке | Одна подпись с сохранением дефиса или разделением слов пробелом; самостоятельные соседние блоки не склеиваются |
| V43 | Отдельные сервисы записаны вложенными HTML-блоками внутри общей нарисованной рамки | Имена и связи отдельных строк сохраняются; объединение многострочной подписи разрешено только при наличии доказательств строки таблицы |
| V44 | Подписи и отдельная строка таблицы имеют общую техническую data-cell группу | Объединение ограничено геометрической областью строки и выравниванием с её ячейками, а не всей технической группой |
